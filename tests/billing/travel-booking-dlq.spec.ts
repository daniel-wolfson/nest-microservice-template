...
describe('TravelBookingSaga - Dead Letter Queue', () => {
  ...
  describe('Compensation Failure Scenarios', () => {
    it('should publish CompensationFailedEvent to Dead Letter Queue when flight cancellation fails', async () => {
      ...
      jest.spyOn(carRentalService, 'reserveCar').mockRejectedValue(
        new Error('No available cars'),
      );
      ...
      const compensationFailedEvents = publishedEvents.filter(
        event => event instanceof CompensationFailedEvent,
      );
    ...
    it('should publish multiple CompensationFailedEvents when multiple compensations fail', async () => {
      ...
      jest.spyOn(saga as any, 'processPayment').mockRejectedValue(
        new Error('Payment processing failed'),
      );

      // Mock ALL cancellations fail
      jest.spyOn(carRentalService, 'cancelCar').mockRejectedValue(
        new Error('Car cancellation API timeout'),
      );
      jest.spyOn(hotelService, 'cancelHotel').mockRejectedValue(
        new Error('Hotel cancellation failed'),
      );
      jest.spyOn(flightService, 'cancelFlight').mockRejectedValue(
        new Error('Flight cancellation failed'),
      );
    ...
    it('should include error stack in CompensationFailedEvent', async () => {
      ...
      // Mock hotel failure
      jest.spyOn(hotelService, 'reserveHotel').mockRejectedValue(
        new Error('Hotel unavailable'),
      );
      ...
      const publishedEvents = publishSpy.mock.calls.map(call => call[0]);
      const compensationFailedEvent = publishedEvents.find(
        event => event instanceof CompensationFailedEvent,
      );
